...
function generateShapePosition(positionsArray, mainElement, dimensions) {
  const position = {};
...
function generateShapesArray(mainElement) {
  const length = 5;
  const tempShapes = [];
  ...
  for (let i = 0; i < length; i++) {
    ...
    const position = generateShapePosition(positionsArray, mainElement, shapeDimension);

    positionsArray.push(position);


    const shape = <RandomShapesSVG
      key={ uuidv4() }
      position={position}
